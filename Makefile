...
help:
	...
	@echo "  make new projectname        - create new project (with templates)"
	...
	@echo ""
	@echo "Examples:"
	@echo "  make new myproject          - create new project"
	@echo "  make clone myproject        - clone project"
	@echo "  make pull analytics         - download changes"
	@echo "  make push myproject         - upload changes"
...
new:
	@echo "ERROR: Project name required. Use: make new projectname"
	@echo "   or: make new name=projectname"
	@echo "   or: make new PROJECT=projectname"
	@exit 1

new-%:
	@echo "Creating new project: $*"
	@node -e "const fs = require('fs'); const path = require('path'); const projectName = '$*'; const projectPath = path.resolve('../' + projectName); if (fs.existsSync(projectPath)) { console.log('ERROR: Project ' + projectName + ' already exists!'); process.exit(1); }"
	@echo "Creating folder structure..."
	@node -e "const fs = require('fs'); const path = require('path'); const projectName = '$*'; const projectPath = path.resolve('../' + projectName); fs.mkdirSync(projectPath, { recursive: true }); fs.mkdirSync(path.join(projectPath, 'system'), { recursive: true }); fs.mkdirSync(path.join(projectPath, 'files'), { recursive: true });"
	@echo "Copying service account..."
	@node -e "const fs = require('fs'); const path = require('path'); const projectName = '$*'; const keyPath = path.resolve('key.json'); const targetPath = path.resolve('../' + projectName + '/system/key.json'); if (fs.existsSync(keyPath)) { fs.copyFileSync(keyPath, targetPath); console.log('✅ Service account copied'); } else { console.log('⚠️  key.json not found (copy manually if needed)'); }"
	@echo "Copying project templates..."
	@node -e "const fs = require('fs'); const path = require('path'); const projectName = '$*'; const templatePath = path.resolve('templates/appsscript.json'); const targetPath = path.resolve('../' + projectName + '/system/appsscript.json'); if (fs.existsSync(templatePath)) { fs.copyFileSync(templatePath, targetPath); console.log('✅ Project template copied'); } else { console.log('⚠️  appsscript.json template not found (copy manually if needed)'); }"
	...
	@echo "  1. cd ../$*"
	@echo "  2. cd ../system"
	@echo "  3. make clone $*  - clone project"
	@echo "  4. cd ../$*"
	@echo "  5. cd ../system"
